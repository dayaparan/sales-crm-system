...
const EditSalesModel = () => {
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        ...
        const { data: res } = await axiosInstance.get(`/user/salesModel/${id}`);
        if (!res.error) {
          const {
            name,
            ownershipType,
  ...
  const onSubmit = async (data) => {
    try {
      const { data: res } = await axiosInstance.put(`/user/salesModel/${id}`, data);
      if (!res.error) {
  ...
  return (
    <Loading loading={isLoading}>
      <Card title={"Edit Sales Model"} headerslot={<BackButton />}>
