...
const AddCategory = () => {
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        ...
        const { data: res } = await axiosInstance.get(`/user/unitType/${id}`);
        if (!res.error) {
          const {
            name,
            projectId,
  ...
  const onSubmit = async (data) => {
    try {
      const { data: res } = await axiosInstance.put(`/user/unitType/${id}`, data);
      if (!res.error) {
  ...
  return (
    <Loading loading={isLoading}>
      <Card title={"Edit Unit Type"} headerslot={<BackButton />}>
