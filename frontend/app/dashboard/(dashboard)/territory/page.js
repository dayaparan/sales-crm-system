...
const UserPage = () => {
  ...
  const getData = async (page, size) => {
    try {
      setIsLoading(true);
      const { data } = await axiosInstance.get("/user/territory", {
        params: { page, limit: size, ...filters },
      });
      if (!data.error) {
        setData(data.territories || []);
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const { data } = await axiosInstance.patch(`/user/territory/${id}`, { status: newStatus });
      if (!data.error) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosInstance.delete(`/user/territory/${id}`);
      if (!data.error) {
        toast.success(data.message);
        getData(pagination.currentPage, pagination.pageSize);
      } else {
            ...
            return (
              <div className="flex space-x-3 rtl:space-x-reverse">
                ...
                <ActionButton
                  title="Edit"
                  icon={"uil:edit"}
                  onClick={() => router.push(`/dashboard/territory/${row.original._id}/edit`)}
                />
                <ActionButton
                  title="Info"
                  icon={"hugeicons:view"}
                  onClick={() => router.push(`/dashboard/territory/${row.original._id}/info`)}
                />
                <ActionButton
                  title="Delete"
  ...
  return (
    <Card>
      <div className="md:flex justify-between items-center mb-6">
        <h4 className="card-title">Territory</h4>
        <div className="flex gap-2">
          ...
          <GlobalFilter filter={globalFilter} setFilter={setGlobalFilter} placeholder="Search Territory..." />
          <AddButton route={"/dashboard/territory/add"} />
        </div>
      </div>
      <TableBody
