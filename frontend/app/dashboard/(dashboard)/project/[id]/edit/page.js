...
const AddCategory = () => {
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        setIsLoading(true);
        const { data: res } = await axiosInstance.get(`/user/project/${id}`);
        if (!res.error) {
          const {
            name,
            brand,
          ...
          reset({
            ...
            totalAcres,
            possessionYear,
            operationalDate: operationalDate?.split("T")[0],
            price,
            size,
            unit,
            breakDown,
  ...
  const onSubmit = async (data) => {
    try {
      const { data: res } = await axiosInstance.put(`/user/project/${id}`, data);
      if (!res.error) {
  ...
  return (
    <Loading loading={isLoading}>
      <Card title={"Edit Project"} headerslot={<BackButton />}>
