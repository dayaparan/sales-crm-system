...
const UserPage = () => {
  ...
  const getData = async (page, size) => {
    try {
      setIsLoading(true);
      const { data } = await axiosInstance.get("/user/branch", { params: { page, limit: size } });
      if (!data.error) {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const { data } = await axiosInstance.patch(`/user/branch/${id}`, { status: newStatus });
      if (!data.error) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosInstance.delete(`/user/branch/${id}`);
      if (!data.error) {
        toast.success(data.message);
        getData(pagination.currentPage, pagination.pageSize);
      } else {
            ...
            return (
              <div className="flex space-x-3 rtl:space-x-reverse">
                ...
                <ActionButton
                  title="Edit"
                  icon={"uil:edit"}
                  onClick={() => router.push(`/dashboard/branch/${row.original._id}/edit`)}
                />
                <ActionButton
                  title="Info"
                  icon={"hugeicons:view"}
                  onClick={() => router.push(`/dashboard/branch/${row.original._id}/info`)}
                />
                <ActionButton
                  title="Delete"
    ...
    <Card>
      <div className="md:flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <GlobalFilter filter={globalFilter} setFilter={setGlobalFilter} placeholder="Search Branches..." />
          <AddButton route={"/dashboard/branch/add"} />
        </div>
      </div>
      <TableBody
