...
const UserPage = () => {
  ...
  const getData = async (page, size) => {
    try {
      setIsLoading(true);
      const { data } = await axiosInstance.get("/user/paymentPlan", {
        params: { page, limit: size, ...filters },
      });
      if (!data.error) {
        setData(data.paymentPlans);
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const { data } = await axiosInstance.patch(`/user/paymentPlan/${id}`, { status: newStatus });
      if (!data.error) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axiosInstance.delete(`/user/paymentPlan/${id}`);
      if (!data.error) {
        toast.success(data.message);
        getData(pagination.currentPage, pagination.pageSize);
      } else {
            ...
            return (
              <div className="flex space-x-3 rtl:space-x-reverse">
                ...
                <ActionButton
                  title="Edit"
                  icon={"uil:edit"}
                  onClick={() => router.push(`/dashboard/paymentplan/${row.original._id}/edit`)}
                />
                <ActionButton
                  title="Info"
                  icon={"hugeicons:view"}
                  onClick={() => router.push(`/dashboard/paymentplan/${row.original._id}/info`)}
                />
                <ActionButton
                  title="Delete"
    ...
    <Card>
      <div className="md:flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          />
          <AddButton route={"/dashboard/paymentplan/add"} />
        </div>
      </div>
      <TableBody
