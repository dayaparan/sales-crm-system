...
const AddLead = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const { data: res } = await axiosInstance.put(`/user/lead/${id}`, data);
      if (!res.error) {
    ...
    const getData = async () => {
      try {
        ...
        const [{ data }, { data: res }] = await Promise.all([
          ...
          axiosInstance.get(`/user/lead/${id}`),
        ]);
        ...
        if (res.success) {
          ...
          reset({
            ...
            lastContactDate: lastContactDate?.split("T")[0],
            nextFollowUpDate: nextFollowUpDate?.split("T")[0],
            preferredChannel,
            campaignEventTag,
            budgetRange,
            timeframe,
  ...
  return (
    <Loading loading={isLoading}>
      <Card title={"Edit Lead"} headerslot={<BackButton />}>
